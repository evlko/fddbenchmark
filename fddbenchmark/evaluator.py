...
class FDDEvaluator:
    ...
    def evaluate(self, labels, pred):
        ...
        real_change_point = (
            labels[labels != 0].reset_index().groupby(["run_id"])["sample"].min()
        )
        pred_change_point = (
            pred[pred != 0].reset_index().set_index(["run_id"])["sample"]
        )
        ...
        valid_change_point = (
            pred_real_change_point["sample_real"] <= pred_real_change_point["sample"]
        )
        pred_change_point = (
            pred_change_point[valid_change_point].groupby(["run_id"]).min()
        )
        ...
        metrics["clustering"]["NMI"] = normalized_mutual_info_score(
            labels.values, pred.values
        )
    ...
    def str_metrics(self, labels, pred):
        ...
        for i in np.arange(labels.max()).astype("int"):
            str_metrics.append(
                "    Fault {:02d}: {:.4f}/{:.4f}".format(
                    i + 1,
                    metrics["classification"]["TPR"][i + 1],
        ...
        str_metrics.append(
            "Average Detection Delay (ADD): {:.2f}".format(metrics["detection"]["ADD"])
        )
        str_metrics.append(
            "Total Correct Diagnosis Rate (Total CDR): {:.4f}".format(
                metrics["diagnosis"]["CDR_total"]
            )
        )
        ...
        str_metrics.append(
            "Normalized Mutual Information (NMI): {:.4f}".format(
                metrics["clustering"]["NMI"]
            )
        )
        str_metrics.append(
            "Unsupervised Clustering Accuracy (ACC): {:.4f}".format(
                metrics["clustering"]["ACC"]
            )
        )
        return "\n".join(str_metrics)
...
def cluster_acc(y_true, y_pred):
    ...
    return (
        sum([w[ind[0][i], ind[1][i]] for i in range(ind[0].size)]) * 1.0 / y_pred.size
    )
